    ...
    <div class="table-container">
        <Table row-key="id" :columns="columns" :data="menus" @on-sort-change="sortTable">
            ...
            <template slot-scope="{ row }" slot="menuType">
                <Tag v-if="row.type == 'DIRECTORY'" color="geekblue">目录</Tag>
                <Tag v-else-if="row.type == 'MENU'" color="green">菜单</Tag>
                <Tag v-else color="blue">按钮</Tag>
            </template>

            <template slot-scope="{ row }" slot="path">
                {{ row.type != 'BUTTON' ? row.path : `[${row.requestMethod}] ${row.path}` }}
            </template>
